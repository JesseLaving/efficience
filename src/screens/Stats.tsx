...
function PostCard({ p }: { p: MetaPost }) {
  const { seedStudio } = useEff();
  /* Un post qui a bien marché doit pouvoir repartir en création : sans ce
     bouton, les statistiques étaient un cul-de-sac — on voyait le succès,
     mais le refaire exigeait un copier-coller manuel vers le Studio. */
  const reuse = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    seedStudio(
      `Rédige une nouvelle publication sur le même sujet que celle-ci, sans la copier : « ${p.caption!.slice(0, 300)} »`,
    );
  };
  return (
    <a
      ...
      <div className="sp-media">
        {p.image ? (
          ...
          <div className="sp-noimg">
            <Icon name={p.network === "instagram" ? "image" : "mail"} />
          </div>
        )}
        {p.type === "video" || p.type === "reels" ? (
      ...
      <div className="sp-body">
        <div className="sp-cap">
          {p.caption ? (
            ...
            <span style={{ color: "var(--tx-3)" }}>Sans légende</span>
          )}
        </div>
        <div className="sp-meta">
          <span>
            <RawIcon svg={heartSvg} />
            {p.likes != null ? fr(p.likes) : "—"}
          </span>
          <span>
            <RawIcon svg={commentSvg} />
            {p.comments != null ? fr(p.comments) : "—"}
          </span>
          {p.shares != null && (
            <span>
              <RawIcon svg={shareSvg} />
              {fr(p.shares)}
            </span>
          )}
          <span className="grow" />
        ...
        {p.caption && (
          <button
            type="button"
            className="sp-reuse"
            title="Repartir de ce sujet dans le Studio"
            onClick={reuse}
          >
            <Icon name="wand" />
            Réutiliser ce sujet
    ...
    <div className="card" style={{ marginBottom: 16 }}>
      <div className="card-h">
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div className="nc-logo" style={{ width: 38, height: 38 }}>
            <Brand name={a.network as BrandName} />
          </div>
          <div>
            <h3>{a.name || a.network}</h3>
            <div className="sub">
              {a.followers != null ? `${fr(a.followers)} abonnés` : "—"}
              {a.mediaCount != null ? ` · ${fr(a.mediaCount)} publications` : ""}
            </div>
          </div>
        </div>
      </div>
      <div className="pad">
        <div
          className="crm-stats"
          style={{ gridTemplateColumns: "repeat(4,1fr)", marginBottom: 4 }}
        >
          <div className="crm-stat">
            ...
            <div className="cs-v">{s.posts}</div>
          </div>
          <div className="crm-stat">
            <div className="cs-l">
              <RawIcon svg={heartSvg} />
            ...
            <div className="cs-v">{fr(s.likes)}</div>
          </div>
          <div className="crm-stat">
            <div className="cs-l">
              <RawIcon svg={commentSvg} />
            ...
            <div className="cs-v">{fr(s.comments)}</div>
          </div>
          <div className="crm-stat">
            <div className="cs-l">
              <Icon name="heart" />
            ...
            <div className="cs-v">
              ...
              {s.engagementRate != null ? (
                ...
                >
                  {s.engagementRate.toFixed(2).replace(".", ",")} %
                </span>
              ) : null}
            </div>
          </div>
          ...
          <div style={{ marginTop: 8, fontSize: 11.5, color: "var(--tx-3)" }}>
            {a.insights.reason ? (
              <>
                ...
                <span style={{ color: "var(--warn)" }}>{a.insights.reason}</span>
              </>
            ) : (
              <>
                Portée &amp; impressions : à activer (
                {a.network === "instagram"
      ...
      {s.posts > 0 ? (
        ...
        <div className="pad" style={{ color: "var(--tx-3)", fontSize: 13 }}>
          {a.postsReason ? (
            <>
              Publications non récupérées —{" "}
              <span style={{ color: "var(--warn)" }}>{a.postsReason}</span>
            </>
          ) : a.network === "facebook" ? (
      ...
      <div className="card-h">
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {me.picture ? (
            ...
            <div className="nc-logo" style={{ width: 38, height: 38 }}>
              <Brand name={"linkedin" as BrandName} />
            </div>
          )}
          <div>
            <h3>{me.name || "Profil LinkedIn"}</h3>
            <div className="sub">
              <Brand name={"linkedin" as BrandName} /> {me.email || "Profil membre connecté"}
            </div>
          </div>
        </div>
      </div>
      <div className="pad" style={{ color: "var(--tx-3)", fontSize: 13 }}>
        Profil membre connecté — vous pouvez publier depuis le{" "}
        <b style={{ color: "var(--tx-2)" }}>Studio</b>. Les statistiques détaillées (abonnés,
        impressions de posts) nécessitent l’API LinkedIn{" "}
        <span style={{ color: "var(--warn)" }}>Community Management</span> (accès partenaire soumis
        à validation) — non disponible pour le moment.
      </div>
    </div>
  );
}
  ...
  return (
    <div className="card" style={{ marginBottom: 16 }}>
      <div className="card-h">
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div className="nc-logo" style={{ width: 38, height: 38 }}>
            <Brand name={"google" as BrandName} />
          </div>
          <div>
            <h3>Google Business</h3>
            <div className="sub">
              {n ? `${n} fiche${n > 1 ? "s" : ""} connectée${n > 1 ? "s" : ""}` : "Compte connecté"}
            </div>
          </div>
        </div>
      </div>
      <div className="pad">
        ...
        <div style={{ marginTop: 8, fontSize: 11.5, color: "var(--tx-3)" }}>
          Vues de la fiche, recherches &amp; avis : nécessitent l’API{" "}
          <span style={{ color: "var(--warn)" }}>Google Business Profile Performance</span> (à
          activer dans votre projet Google Cloud).
        </div>
      </div>
    </div>
  );
    ...
    <section className="screen show anim">
      <div className="page-head">
        <div>
          ...
          <p>
            Données réelles importées via les API officielles de vos réseaux connectés : Instagram
            &amp; Facebook (publications, j’aime, commentaires, partages), LinkedIn et Google
            Business. Certaines métriques avancées (portée, vues de fiche, abonnés LinkedIn)
            nécessitent des accès API supplémentaires.
          </p>
        </div>
        {metaConnected && (
          <button className="btn outline" onClick={refreshMetaStats} disabled={loading}>
            {loading ? (
              <>
      ...
      {!anyConnected && (
        <div className="net-summary">
          ...
          <div>
            ...
            <div className="ns-s">
              Connectez Instagram, Facebook, LinkedIn ou Google Business pour voir vos statistiques
              réelles.
            </div>
          </div>
          <button
      ...
      {error && (
        <div
          style={{
            ...
            border: "1px solid rgba(179,69,59,.35)",
            background: "rgba(179,69,59,.08)",
            color: "var(--danger)",
            fontSize: 13,
            marginBottom: 16,
          }}
      ...
      {metaConnected && loading && !accounts && (
        <div style={{ color: "var(--tx-3)", fontSize: 13, padding: 24, textAlign: "center" }}>
          <span className="spin lt" style={{ display: "inline-block", marginRight: 8 }} />
          Récupération de vos publications…
        </div>
      )}
