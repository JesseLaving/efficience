...
const escapeHtml = (s: string) =>
  s.replace(/[&<>]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;" })[c]!);
const escapeTags = (s: string) =>
  escapeHtml(s).replace(/#([\wàâäéèêëîïôöùûüç-]+)/gi, '<span class="tags">#$1</span>');
...
function ratioBox(r: string) {
  ...
  return (
    <span
      className="rbox"
      style={{ width: (w * s).toFixed(1) + "px", height: (h * s).toFixed(1) + "px" }}
    />
  );
}
...
export function Studio() {
  ...
  const initSel = (t: ComposeType) => {
    ...
    let r = ratio;
    if (a) {
      const rs = SPECS[t === "email" ? "post" : t][a].ratios;
      if (!rs.includes(r)) r = rs[0];
    }
  ...
  const togglePlat = (id: string) => {
    ...
    if (i >= 0) {
      if (nextSel.length > 1) nextSel.splice(i, 1);
  ...
  const loadMedia = (f: File) => {
    ...
    rd.onload = () => {
      setMedia({ url: rd.result as string, kind, name: f.name, size: f.size });
  ...
  const runAi = async (mode: "post" | "improve") => {
    ...
    if (mode === "improve" && !brief) {
      showToast(UI.close, "Écrivez d’abord un texte à améliorer.");
      return;
    }
    setAiBusy(mode);
    setVariants(null);
    try {
      ...
      const ctx = {
        ...
        sector: b.sector,
        city: b.city,
        network: active ? netName(active) : undefined,
        tone: tone || undefined,
        maxLength: curLimit() || undefined,
        audience: strat?.audience || undefined,
        products: strat?.products || undefined,
        goal: strat?.goal || undefined,
        competitors: strat?.competitors || undefined,
        recentPosts: [
      ...
      const res =
        mode === "improve"
          ...
          : await generatePost(
              brief || `Une publication pour ${b.name} — secteur ${b.sector}`,
              ctx,
            );
  ...
  const runHashtags = async () => {
    const brief = text.trim();
    if (!brief) {
      showToast(UI.close, "Écrivez d’abord un texte pour générer des hashtags pertinents.");
      return;
    }
    setAiBusy("hashtags");
    try {
  ...
  const schedule = () => {
    ...
    if (editingId && prev) {
      ...
      if (prev?.auto) {
        ...
        showToast(
          UI.warning,
          "Post mis à jour — auto-publication désactivée. Réactivez-la au calendrier pour armer le nouveau contenu.",
        );
      } else {
  ...
  const saveDraftNow = () => {
    ...
    if (!hasContent) {
      showToast(UI.close, "Rien à enregistrer pour le moment.");
      return;
    }
    const d: Draft = {
      id: `d_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`,
      type,
      text: type === "email" ? body : text,
      networks: type === "email" ? [] : sel,
      ratio,
      photoUrl: publicImageUrl,
  ...
  const loadDraft = (d: Draft) => {
    ...
    if (d.type === "email") {
      ...
      if (d.photoUrl) {
        setMedia({ url: d.photoUrl, kind: "image", name: "brouillon.jpg", size: 0 });
        setPublicImageUrl(d.photoUrl);
      } else {
        setMedia(null);
        setPublicImageUrl(null);
      }
    ...
    <section className="screen show anim">
      <div className="page-head" style={{ marginBottom: 22 }}>
        <div>
          ...
          <p>
            Rédigez vos publications, stories et e-mails à la main. Efficience vérifie en direct les
            limites de texte et les formats visuels propres à chaque plateforme.
          </p>
        </div>
      </div>
      ...
      <div className="ce-sec" style={{ border: "none", padding: "0 0 18px" }}>
        <div className="switch">
          <button
            className={"switch-btn" + (type === "post" ? " on" : "")}
          ...
          <button
            className={"switch-btn" + (type === "story" ? " on" : "")}
          ...
          <button
            className={"switch-btn" + (type === "email" ? " on" : "")}
      ...
      <div className="cmp">
            ...
            <>
              <div className="ce-sec">
                <label className="field-lbl">
                  Plateformes {type === "story" ? "(stories)" : ""}
                </label>
                <div className="plat-row">
                  {availablePlatforms(type).map((id) => {
                    ...
                    return (
                      <button
                        key={id}
                        className={"plat-chip" + (sel.includes(id) ? " on" : "")}
                        onClick={() => togglePlat(id)}
                      ...
                      >
                        ...
                        {pending && (
                          <span
                            className="plat-pending-dot"
                            aria-label={PUBLISH_STATUS_REASON[status]}
                          />
                        )}
                ...
                {sel.some((id) => PUBLISH_STATUS[id] && PUBLISH_STATUS[id] !== "ready") && (
                  <div className="counter" style={{ marginTop: 8, color: "var(--tx-3)" }}>
                    ...
                    />
                    {sel
                      .filter((id) => PUBLISH_STATUS[id] && PUBLISH_STATUS[id] !== "ready")
                      .map((id) => `${netName(id)} : ${PUBLISH_STATUS_REASON[PUBLISH_STATUS[id]]}`)
                      .join(" · ")}
                  </div>
                )}
              </div>
              ...
              <div className="ce-sec">
                ...
                <div className="cmp-text">
                  <div className="ai-tone-row">
                    ...
                    {TONES.map((t) => (
                      <button
                        key={t}
                        type="button"
                        className={"ai-tone-chip" + (tone === t ? " on" : "")}
                  ...
                  <div className="counter-row">
                    <div>
                      {(() => {
                        ...
                        return (
                          <>
                            <div className={"counter " + st} style={{ marginBottom: 6 }}>
                              <b>{fr(len)}</b> / {fr(lim)} caractères{" "}
                              {len > lim && (
                                <>
                                  · <span>{fr(len - lim)} en trop</span>
                                </>
                              )}
                            </div>
                            <div className={"counter-bar " + st} style={{ width: 200 }}>
                              <i style={{ width: pct + "%" }} />
                            </div>
                          </>
                        );
                    ...
                    <div className="cmp-tools">
                      <button title="Emoji" onClick={insertEmoji}>
                        😊
                      </button>
                      <button
                        title="Générer des hashtags par IA"
                        disabled={!!aiBusy}
                        onClick={runHashtags}
                      >
                        {aiBusy === "hashtags" ? <span className="spin lt" /> : <Icon name="tag" />}
                      </button>
                      <button
                        title="Rédiger par IA"
                        disabled={!!aiBusy}
                        onClick={() => runAi("post")}
                      >
                        {aiBusy === "post" ? <span className="spin lt" /> : <Icon name="wand" />}
                      </button>
                      <button
                        title="Améliorer le texte par IA"
                        disabled={!!aiBusy}
                  ...
                  {/* Pendant la génération : le squelette occupe l'emplacement
                      exact du résultat (versions ou hashtags) — le contenu le
                      remplace sans faire sauter la page. */}
                  {aiBusy && (
                    <div className="ai-variants">
                      <AiLoader
                        ...
                        phrases={
                          aiBusy === "post"
                            ? [
                                "Lecture de votre marque et de vos dernières publications…",
                                "Rédaction de plusieurs versions…",
                                "Ajustement au réseau et à la longueur…",
                              ]
                            : aiBusy === "improve"
                              ? [
                              ...
                              : [
                                  "Analyse des mots-clés de votre texte…",
                                  "Sélection des hashtags pertinents…",
                                ]
                        }
                      />
                      {aiBusy === "hashtags" ? (
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                          {[76, 98, 64, 110, 58].map((w, i) => (
                            <Skel key={i} w={w} h={24} r={999} />
                          ))}
                        </div>
                      ) : (
                        Array.from({ length: aiBusy === "post" ? 2 : 1 }, (_, i) => (
                          <div className="ai-variant" key={i}>
                            <div className="ai-v-head">
                              <Skel w={74} h={10} />
                              <Skel w={92} h={10} style={{ marginLeft: "auto" }} />
                            </div>
                            <SkelText lines={3} />
                          </div>
                        ))
                  ...
                  {variants && (
                    <div className="ai-variants">
                      <div className="ai-variants-lbl">
                        <RawIcon
                          svg={UI.sparkles2}
                          style={{ width: 13, height: 13, display: "inline-grid" }}
                        />
                      ...
                      {variants.map((v, i) => {
                        ...
                        const len = v.length;
                        const st =
                          lim && len > lim ? "over" : lim && len > lim * 0.85 ? "warn" : "ok";
                        return (
                            ...
                            <div className="ai-v-actions">
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  showToast(UI.check, "Version copiée");
                                }}
                              >
                                <Icon name="clipboard" />
                              </button>
                      ...
                      <button className="btn ghost sm" onClick={() => setVariants(null)}>
                        Ignorer
                      </button>
                    </div>
                  )}
              ...
              <div className="ce-sec">
                ...
                <div className="ratio-row">
                  {(spec?.ratios || []).map((r) => (
                    <button
                      key={r}
                      className={"ratio-chip" + (r === ratio ? " on" : "")}
                      onClick={() => setRatio(r)}
                    >
                      {ratioBox(r)}
                      {r}
                ...
                <div className="spec-grid">
                  <div className="spec-row">
                    <Icon name="image" />
                    <div>
                      <div className="sr-l">Dimensions conseillées</div>
                      <div className="sr-v">{spec?.rec || "—"}</div>
                    </div>
                  </div>
                  <div className="spec-row">
                    <Icon name="sheet" />
                    <div>
                      <div className="sr-l">Image · Vidéo</div>
                      <div className="sr-v">
                        {spec?.img || "—"} · {spec?.video || "—"}
                      </div>
                    </div>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 10 }}>
                  <button
                    ...
                    title="Composer un visuel aux couleurs de votre marque"
                  >
                    <RawIcon svg={UI.sparkles2} />
                    Générer un visuel de marque
                  </button>
                </div>
                <div>
                  {media ? (
                    <div className="media-has">
                      ...
                      >
                        {media.kind === "video" && (
                          <Icon name="play" style={{ width: 20, height: 20 }} />
                        )}
                      </div>
                      <div className="mh-i">
                        <div className="mh-n">{media.name}</div>
                        <div className="mh-m">
                          {media.kind === "video" ? "Vidéo" : "Image"} · format {ratio} ·{" "}
                          {netName(active || "")}
                        </div>
                      </div>
                      <button
                    ...
                    >
                      ...
                      <div className="md-s">
                        Image ou vidéo · recadrée en {ratio} pour {netName(active || "")}
                      </div>
                      <input
                        type="file"
                        ref={fileRef}
                    ...
                    sel.map((id) => {
                      ...
                      return (
                        <div className="compl-row" key={id}>
                          ...
                          <div className={"cr-bar" + (over ? " over" : "")}>
                            <i style={{ width: pct + "%" }} />
                          </div>
                          {!lim ? (
                            <span className="compl-st ok">
                              <RawIcon svg={UI.check} />
                              texte libre
                            </span>
                          ) : over ? (
                            <span className="compl-st over">
                              <RawIcon svg={UI.close} />−{fr(len - lim)}
                            </span>
                          ) : (
                            <span className="compl-st ok">
                              <RawIcon svg={UI.check} />
              ...
              {editingId && (
                ...
                >
                  ...
                  <span style={{ flex: 1, minWidth: 200 }}>
                    <b style={{ color: "var(--acc)" }}>Modification d’un post programmé</b> — «
                    Mettre à jour » remplacera son contenu au calendrier.
                  </span>
                  <button className="btn ghost sm" onClick={() => setEditingId(null)}>
                    Créer un nouveau post à la place
                  </button>
                </div>
              )}
              ...
              {schedOpen && (
                <div className="ce-sec" style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <label className="field-lbl" style={{ margin: 0, flexShrink: 0 }}>
                    {editingId ? "Reprogrammer pour" : "Programmer pour"}
                  </label>
                  <input
                    type="datetime-local"
                    className="inp"
                  ...
                  <button
                    ...
                    disabled={!text.trim() || !sel.length}
                    onClick={schedule}
                  >
                    <Icon name="check" />
                    {editingId ? "Mettre à jour" : "Confirmer"}
                  </button>
                  <button className="btn ghost sm" onClick={() => setSchedOpen(false)}>
                    Annuler
                  </button>
                </div>
              )}
              ...
              >
                <button className="btn outline" onClick={saveDraftNow}>
                            ...
                            <div className="notif-item" key={d.id} style={{ alignItems: "center" }}>
                              ...
                              >
                                <div className="notif-text">
                                  {(d.type === "email" ? d.subject : d.text)?.slice(0, 70) || (
                                    <em>Sans texte</em>
                                  )}
                                  {(d.type === "email" ? d.subject : d.text) &&
                                  (d.type === "email" ? d.subject! : d.text).length > 70
                                    ? "…"
                                    : ""}
                                </div>
                ...
                {!anyConnected && (
                  <span style={{ fontSize: 12, color: "var(--tx-3)", whiteSpace: "nowrap" }}>
                    Reliez un réseau pour publier.
                  </span>
                )}
                <span style={{ flex: 1 }} />
                <button
                ...
                >
                  <Icon name="clock" />
                  {editingId ? "Mettre à jour" : "Programmer"}
                </button>
                <button
                  className="btn acc"
                  disabled={!text.trim() || !anyConnected}
        ...
        <div className="cmp-prev">
          <div className="pv-bar">
            <Icon name="eye" />
            <span className="pvb-t">Aperçu</span>
            <span className="pvb-net">
              {type === "email" ? (
                <Icon name="mail" />
              ) : active ? (
                <Brand name={active as BrandName} />
              ) : null}
              {type === "email" ? "E-mail" : netName(active || "")}
            </span>
          </div>
          <div className="pv-stage">
            {type === "email" ? (
              <EmailPreviewCard subject={subject} pre={pre} body={body} />
            ) : type === "story" ? (
              <StoryPreview text={text} active={active} media={media} />
            ) : (
      ...
      {publishOpen && (
        <PublishPanel
          ...
          platforms={sel}
          localMedia={!!media}
          defaultPhotoUrl={publicImageUrl}
        ...
        <VisualGenerator
          ...
          onUse={(url, size, isPublic) => {
            setMedia({
              url,
              kind: "image",
              name: isPublic ? "photo-pexels.jpg" : "visuel-de-marque.png",
              size,
            });
  ...
  return (
    <div className="ig-card">
      <div className="ig-head">
        ...
        <div>
          <div className="ih-n">{handleFor(active || "")}</div>
          <div className="ih-s">{getBusiness().city} · Sponsorisé</div>
        </div>
        <div className="ih-net">{active && <Brand name={active as BrandName} />}</div>
      </div>
      <div className="ig-media" style={{ aspectRatio: ratio.replace(":", "/"), ...bg }}>
        {mediaInner(media, ratio)}
      </div>
      <div className="ig-actions">
        <RawIcon svg={A.heart} />
        <RawIcon svg={A.comment} />
        <RawIcon svg={A.share} />
        <span className="grow" />
        <RawIcon svg={A.bookmark} />
      </div>
      <div className="ig-cap">
        {txt ? (
          <span
            dangerouslySetInnerHTML={{
              __html: `<b>${handleFor(active || "")}</b> ${escapeTags(txt)}`,
            }}
          />
        ) : (
          <span className="empty">Votre légende apparaîtra ici…</span>
        )}
      </div>
    </div>
  );
...
function StoryPreview({
  ...
  return (
    <div className="story-card" style={bg}>
      <div className="st-grad-top" />
      <div className="st-grad-bot" />
      ...
      <div className="st-head">
        <div className="ava">{getBusiness().initials}</div>
        <div>
          <div className="sh-n">{handleFor(active || "")}</div>
          <div className="sh-t">il y a 2 min</div>
        </div>
      </div>
      {!media && (
        <div className="st-empty">
          <Icon name="image" />
          <small>Visuel 9:16</small>
        </div>
      )}
      <div className="st-cap">
        {txt ? <span dangerouslySetInnerHTML={{ __html: escapeTags(txt) }} /> : null}
      </div>
    </div>
  );
}
...
function EmailPreviewCard({ subject, pre, body }: { subject: string; pre: string; body: string }) {
  const b = (body || "").trim();
  const paras = b ? (
    b.split(/\n+/).map((p, i) => <p key={i} dangerouslySetInnerHTML={{ __html: escapeHtml(p) }} />)
  ) : (
    <p style={{ color: "#bbb" }}>Le corps de votre e-mail s’affiche ici…</p>
  );
  return (
    <div style={{ width: "100%", maxWidth: 380 }}>
      <div className="ep-mail">
        ...
        <div className="ep-subj-line">
          {subject ? subject : <span style={{ color: "#bbb" }}>Objet de l’e-mail</span>}
        </div>
        <div style={{ fontSize: 12, color: "#999", padding: "4px 20px 0" }}>
          {pre || "Pré-en-tête…"}
        </div>
        <div className="ep-head-band">
          <img src={`${import.meta.env.BASE_URL}assets/logo-white.png`} alt="Efficience" />
        </div>
        <div className="ep-body-c">{paras}</div>
        <div className="ep-foot">
          <div className="ef-social">
            {(["instagram", "facebook", "tiktok"] as BrandName[]).map((s) => (
              <span key={s}>
                <Brand name={s} />
              </span>
            ))}
  ...
  const counter = (len: number, lim: number) => {
    ...
    return (
      <div className={"counter " + s}>
        <b>{len}</b> / {lim}{" "}
        {len > lim ? "· trop long pour certaines messageries" : "· longueur idéale"}
      </div>
    );
  };
  return (
    <>
      <div className="ce-sec">
        ...
        <input
          className="inp"
          maxLength={120}
          placeholder="Ex : votre prochaine session de formation — places limitées"
          value={subject}
      ...
      <div className="ce-sec">
        ...
        <input
          className="inp"
          maxLength={140}
          placeholder="Une phrase d’accroche affichée après l’objet"
          value={pre}
      ...
      <div className="ce-sec">
        ...
        <div className="counter" style={{ marginTop: 8 }}>
          Largeur d’image conseillée <b>600 px</b> · texte alternatif requis
        </div>
      </div>
      <div
        className="ce-sec"
      ...
      >
        ...
        <button
          className="btn acc"
          disabled
          title="L’envoi réel n’est pas encore connecté — rédigez et enregistrez votre brouillon en attendant."
        >
          <Icon name="send" />
          Programmer l’envoi
        </button>
