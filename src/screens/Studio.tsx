...
const handleFor = (id: string): string =>
  (({ instagram: '@boulangerie.martin', facebook: 'Boulangerie Martin', linkedin: 'Boulangerie Martin', x: '@boul_martin', tiktok: '@boulangerie.martin', youtube: '@boulangeriemartin', pinterest: '@boulangeriemartin', google: 'Boulangerie Martin' } as Record<string, string>)[id]) || '@boulangerie.martin';
...
export function Studio() {
  ...
  const insert = (k: 'emoji' | 'hash' | 'ai') => {
    const map = { emoji: ' 🥐', hash: ' #boulangerie #faitmaison', ai: ' Découvrez nos fournées du matin, encore tièdes ✨' };
    setText((t) => t + map[k]);
  };
...
function FeedPreview({ text, active, ratio, media }: { text: string; active: string | null; ratio: string; media: Media | null }) {
  ...
  return (
    <div className="ig-card">
      <div className="ig-head"><div className="ava">BM</div><div><div className="ih-n">{handleFor(active || '')}</div><div className="ih-s">Lyon 3e · Sponsorisé</div></div><div className="ih-net">{active && <Brand name={active as BrandName} />}</div></div>
...
function StoryPreview({ text, active, media }: { text: string; active: string | null; media: Media | null }) {
  ...
  return (
    <div className="story-card" style={bg}>
      ...
      <div className="st-head"><div className="ava">BM</div><div><div className="sh-n">{handleFor(active || '')}</div><div className="sh-t">il y a 2 min</div></div></div>
  ...
  return (
    <>
      <div className="ce-sec">
        ...
        <input className="inp" maxLength={120} placeholder="Ex : Vos viennoiseries préférées en édition limitée" value={subject} onChange={(e) => setSubject(e.target.value)} />
