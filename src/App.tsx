...
interface NavItem { screen: ScreenId; icon: UIName; label: string; sub?: string }
...
const GROUPS: { label: string; items: NavItem[] }[] = [
  ...
  { label: 'Créer & publier', items: [
    ...
    { screen: 'planning', icon: 'calendar', label: 'Planning éditorial', sub: 'Les sujets du mois' },
  ...
  { label: 'Mesurer', items: [
    ...
    { screen: 'stats', icon: 'chart', label: 'Analyse entreprise & site', sub: 'INSEE & audit du site' },
  ] },
];
...
export function App({ headerLeft, headerRight }: { headerLeft?: React.ReactNode; headerRight?: React.ReactNode } = {}) {
  ...
  if (!setup.complete) {
    setup.steps
      .filter((s) => !s.optional)
      .forEach((s, idx) => stepByScreen.set(s.screen, {
        n: idx + 1, done: s.done, next: setup.next?.key === s.key,
      }));
  }
      ...
      <aside className={'side' + (navOpen ? ' open' : '')}>
        {GROUPS.map((g) => (
          <div className="nav-grp" key={g.label}>
            ...
            {g.items.filter((it) => it.screen !== 'config' || !setup.complete).map((it) => {
              const st = stepByScreen.get(it.screen);
              return (
              <button
                key={it.screen}
                type="button"
                className={'nav-i' + (screen === it.screen ? ' active' : '') + (it.sub ? ' has-sub' : '') + (st?.next ? ' step-next' : '')}
                aria-current={screen === it.screen ? 'page' : undefined}
                onClick={() => go(it.screen)}
              >
                {st
                  ? <span className={'nav-step' + (st.done ? ' done' : '')} aria-hidden="true">{st.done ? <Icon name="check" /> : st.n}</span>
                  : <Icon name={it.icon} />}
