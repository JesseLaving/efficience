...
export function VisualGenerator({ text, ratio, onClose, onUse }: Props) {
  ...
  const [aiPrompt, setAiPrompt] = useState(() =>
    aiImagePrompt(text, getBusiness().sector, "photo", brandKit.palette),
  );
  const [aiUrl, setAiUrl] = useState<string | null>(null);
  const [aiLoading, setAiLoading] = useState(false);
  const [aiProvider, setAiProvider] = useState<"gemini" | "cloudflare" | "pollinations" | null>(
    null,
  );
    ...
    try {
      ...
      setPreason(
        d.available
          ...
          : d.reason || "Recherche indisponible.",
      );
    } catch (e) {
      setPreason(String((e as Error).message || e));
    }
    setPloading(false);
  };
  ...
  const svg = useMemo(
    () => buildVisual({ template, brand: kit, text, ratio, title, logoData }),
    [template, kit, text, ratio, title, logoData],
  );
  ...
  const use = async () => {
    if (mode === "ai") {
      if (!aiUrl) {
        showToast(UI.close, "Générez d’abord une image");
        return;
      }
      ...
      try {
        ...
        if (dataUrl) {
          ...
          if (up.ok && up.url) {
            setBusy(false);
            onUse(up.url, 0, true);
            showToast(UI.check, "Image IA ajoutée (URL publique)");
            onClose();
            return;
          }
        }
        ...
        if (isDataUrl) {
          showToast(
            UI.close,
            "Hébergement d’image indisponible — impossible d’utiliser cette image IA pour l’instant.",
          );
          return;
        }
        // Repli : l'URL Pollinations est publique et déterministe → publiable.
        onUse(aiUrl, 0, true);
        showToast(UI.check, "Image IA ajoutée");
        onClose();
        return;
      } catch {
        setBusy(false);
        ...
        onUse(aiUrl, 0, true);
        showToast(UI.close, "Compositing impossible — image brute utilisée.");
        onClose();
        return;
      }
    }
    if (mode === "photo") {
      if (!selPhoto) {
        showToast(UI.close, "Sélectionnez une photo");
        return;
      }
      if (brandPhotoOn) {
        setBusy(true);
        try {
          const dataUrl = await brandPhoto({
            photoUrl: selPhoto.url,
            ratio,
            logoData,
            brandName: kit.name || undefined,
            accent: kit.accent || undefined,
          ...
          if (up.ok && up.url) {
            onUse(up.url, 0, true);
            showToast(UI.check, "Visuel brandé ajouté (URL publique)");
            onClose();
            return;
          }
          ...
          showToast(
            UI.close,
            (up.reason || "Hébergement indisponible") + " — photo non brandée utilisée.",
          );
          onUse(selPhoto.url, 0, true);
          onClose();
          return;
        } catch {
          setBusy(false);
    ...
    try {
      ...
      showToast(UI.close, "Échec de la génération du visuel");
    }
    setBusy(false);
  };
  ...
  const download = async () => {
    if (mode === "ai") {
      ...
      if (aiUrl.startsWith("data:")) {
        ...
        a.href = aiUrl;
        a.download = "image-ia-efficience.png";
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = png;
      a.download = "visuel-" + template + ".png";
      document.body.appendChild(a);
      a.click();
      a.remove();
    } catch {
      showToast(UI.close, "Échec du téléchargement");
    }
  };

  const switchToPhoto = () => {
    setMode("photo");
    if (!photos.length && !ploading) searchPhotos(pquery);
  };
  const modeBtn = (on: boolean): React.CSSProperties => ({
    ...
    color: on ? "var(--on-acc)" : "var(--tx-2)",
  });

  const fileRef = useRef<HTMLInputElement>(null);
  const onLogoFile = (f: File) => {
    const fr = new FileReader();
    fr.onload = () => patch({ logo: fr.result as string });
    fr.readAsDataURL(f);
  };
      ...
      <div className="kmodal-card" style={{ maxWidth: 900, width: "94vw" }}>
        <div className="kmodal-top">
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <RawIcon
              svg={UI.sparkles2}
              style={{ width: 18, height: 18, display: "inline-grid", color: "var(--acc)" }}
            />
            <b>Générateur de visuel de marque</b>
          </div>
          <button className="km-x" aria-label="Fermer" onClick={onClose}>
            <Icon name="close" />
          </button>
        </div>
        ...
        <div className="kmodal-body" style={{ display: "block" }}>
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            <button onClick={() => setMode("template")} style={modeBtn(mode === "template")}>
              <RawIcon
                svg={UI.sparkles2}
                style={{ width: 14, height: 14, display: "inline-grid", verticalAlign: -2 }}
              />{" "}
              Modèle de marque
            </button>
            <button onClick={switchToPhoto} style={modeBtn(mode === "photo")}>
              <RawIcon
                svg={UI.image}
                style={{ width: 14, height: 14, display: "inline-grid", verticalAlign: -2 }}
              />{" "}
            ...
            <button onClick={switchToAi} style={modeBtn(mode === "ai")}>
              <RawIcon
                svg={UI.wand}
                style={{ width: 14, height: 14, display: "inline-grid", verticalAlign: -2 }}
              />{" "}
              ...
              <div>
                  ...
                  {TEMPLATES.map((t) => (
                    <button
                      ...
                      style={{
                        ...
                        color: template === t.key ? "var(--on-acc)" : "var(--tx-2)",
                      }}
                    >
                      {t.label}
                    </button>
                ...
                {template === "annonce" && (
                  <input
                    ...
                    placeholder="Titre / accroche (ex : À la une)"
                  />
                )}
                <div style={{ fontSize: 11.5, color: "var(--tx-3)", marginTop: 10 }}>
                  Le visuel reprend le texte de votre publication et votre charte ({ratio}).
                  Modifiez le texte dans le composer pour le mettre à jour.
                </div>
              </div>
              ...
              <div style={{ display: "grid", gap: 14, alignContent: "start" }}>
                <div className="field">
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <button
                      ...
                      onClick={() => refreshBrand(siteUrl)}
                      title="Ré-extraire la charte du site"
                    >
                      {brandStatus === "loading" ? (
                  ...
                  <div style={{ fontSize: 11, color: "var(--tx-3)", marginTop: 6 }}>
                    {kit.available
                      ? "Couleurs, logo et police extraits de votre site."
                      : "Charte par défaut (Efficience). Lancez l’extraction pour utiliser celle de votre site."}
                  </div>
                </div>
                ...
                <div className="field">
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {kit.palette.map((c, i) => (
                      <button
                        key={c + i}
                        title={"Définir comme couleur d’accent : " + c}
                        ...
                        style={{
                          ...
                          background: c,
                          border:
                            "2px solid " + (kit.accent === c ? "var(--tx-str)" : "transparent"),
                ...
                <div className="field">
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    {kit.logo && (
                      <button className="btn ghost sm" onClick={() => patch({ logo: null })}>
                        <Icon name="trash" />
                      </button>
                    )}
                    <input
                  ...
                  {kit.logo && !logoData && (
                    <div style={{ fontSize: 11, color: "var(--warn)", marginTop: 5 }}>
                      Logo affiché mais non intégrable à l’export (CORS) — il sera remplacé par le
                      nom de marque dans le PNG.
                    </div>
                  )}
                </div>
                ...
                <div className="field">
                  <label className="field-lbl">
                    Police {kit.fonts[0] ? `· ${kit.fonts[0]}` : ""}
                  </label>
                  <input
                    className="inp"
                    value={kit.fonts[0] || ""}
                    onChange={(e) => patch({ fonts: [e.target.value, ...kit.fonts.slice(1)] })}
                    placeholder="Inter"
                  />
                </div>
                ...
                <button
                  ...
                  onClick={switchToAi}
                  title="Générer une image par IA (gratuit)"
                >
                  <Icon name="wand" />
            ...
            <div>
              <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") searchPhotos(pquery);
                  }}
                  placeholder="Rechercher une photo (ex : formation réunion)"
                />
                <button
                  className="btn acc sm"
                  disabled={ploading}
                  onClick={() => searchPhotos(pquery)}
                  style={{ flexShrink: 0 }}
                >
                  {ploading ? <span className="spin" /> : <Icon name="search" />}Rechercher
                </button>
              </div>
              <label
                style={{
              ...
              >
                <input
                  type="checkbox"
                  checked={brandPhotoOn}
                  onChange={(e) => setBrandPhotoOn(e.target.checked)}
              ...
              {preason && (
                <div style={{ fontSize: 12.5, color: "var(--warn)", marginBottom: 12 }}>
                  {preason}
                  {/Clé Pexels/.test(preason) ? (
                    <span style={{ color: "var(--tx-3)" }}>
                      {" "}
                      — ajoutez la variable <b>PEXELS_API_KEY</b> dans Vercel.
                    </span>
                  ) : null}
                </div>
              )}
              ...
              >
                {photos.map((p) => (
                  <button
                    key={p.id}
                    onClick={() => setSelPhoto(p)}
                    title={p.alt}
                    style={{
                      padding: 0,
                      border:
                        "2px solid " +
                        (selPhoto && selPhoto.id === p.id ? "var(--acc)" : "transparent"),
                      borderRadius: 10,
                      overflow: "hidden",
                      cursor: "pointer",
                      aspectRatio: "1 / 1",
                      background: p.avgColor || "var(--canvas-soft)",
                    }}
                  >
                    <img
                      src={p.thumb}
              ...
              {!photos.length && !ploading && !preason && (
                <div style={{ fontSize: 12.5, color: "var(--tx-3)" }}>
                  Lancez une recherche pour illustrer votre sujet en photo.
                </div>
              )}
              {selPhoto && (
                <div style={{ marginTop: 12, fontSize: 11.5, color: "var(--tx-3)" }}>
                  {selPhoto.photographer ? (
                    <>
                      Photo ©{" "}
                      <a
                        href={selPhoto.photographerUrl || "#"}
                        target="_blank"
                        rel="noopener"
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 20 }}>
              ...
              <div>
                ...
                >
                  {aiUrl ? (
                    ...
                    /* Reflet animé à la taille exacte de l'image attendue :
                     l'aperçu ne reste jamais un cadre vide pendant le rendu. */
                    <Skel w="100%" h="100%" r={0} style={{ position: "absolute", inset: 0 }} />
                  ) : (
                    <div
                      style={{
                        fontSize: 12.5,
                ...
                <div style={{ fontSize: 11.5, color: "var(--tx-3)", marginTop: 10 }}>
                  {aiProvider === "gemini"
                    ? `Image générée par Gemini (Google)${aiModel ? ` · ${aiModel}` : ""} — haute qualité.`
                    : aiProvider === "cloudflare"
                      ? `Image générée par Cloudflare Workers AI (gratuit)${aiModel ? ` · ${aiModel.split("/").pop()}` : ""} — générée directement au format ${ratio}.`
                      : aiProvider === "pollinations"
                        ? "Image générée par le moteur gratuit (Pollinations)."
                ...
                {aiFallbackReason && (
                  <div
                    style={{
                      fontSize: 11,
                      color: aiProvider === "gemini" ? "var(--danger)" : "var(--tx-3)",
                      marginTop: 4,
                    }}
                  >
                    {aiProvider === "gemini"
                ...
                {aiProvider === "gemini" && referencePhoto && (
                  <div style={{ fontSize: 11, color: "var(--tx-3)", marginTop: 4 }}>
                    Style inspiré de votre dernière publication publiée, pour une identité visuelle
                    cohérente.
                  </div>
                )}
              </div>
              ...
              <div style={{ display: "grid", gap: 14, alignContent: "start" }}>
                <div className="field">
                  ...
                  <div className="km-fmt-row">
                    {IMAGE_STYLES.map((s) => (
                      <button
                        key={s.id}
                        type="button"
                        className={"km-fmt" + (aiStyle === s.id ? " on" : "")}
                  ...
                  <div style={{ fontSize: 11, color: "var(--tx-3)", marginTop: 6 }}>
                    Réécrit le prompt avec une direction artistique et vos couleurs de marque.
                  </div>
                </div>
                <div className="field">
                  ...
                  <div className="km-fmt-row">
                    {(
                      ...
                      <button
                        key={id}
                        type="button"
                        className={"km-fmt" + (aiEngine === id ? " on" : "")}
                        onClick={() => setAiEngine(id)}
                      >
                        {label}
                      </button>
                    ))}
                  ...
                  <div style={{ fontSize: 11, color: "var(--tx-3)", marginTop: 6 }}>
                    {aiEngine === "auto"
                      ? "Gemini si disponible, sinon repli sur le moteur gratuit."
                      : aiEngine === "gemini"
                        ? "Gemini uniquement — aucune bascule silencieuse en cas d’erreur."
                ...
                <div className="field">
                  ...
                  <textarea
                    ...
                    rows={5}
                    placeholder="Ex : photographie d’un atelier de formation lumineux, style moderne"
                ...
                <button
                  ...
                  onClick={generateAi}
                >
                  {aiLoading ? <span className="spin" /> : <Icon name="wand" />}
                  {aiUrl ? "Régénérer" : "Générer l’image"}
                </button>
                <label
                  style={{
                    display: "flex",
        ...
        <div className="kmodal-foot">
          <span className="grow" style={{ fontSize: 12, color: "var(--tx-3)" }}>
            {mode === "photo"
              ? "Photos Pexels — URL publique, publiable directement."
              : mode === "ai"
                ? aiProvider === "pollinations"
          ...
          <button
            className="btn outline"
            onClick={download}
            disabled={(mode === "photo" && !selPhoto) || (mode === "ai" && !aiUrl)}
          >
            <Icon name="download" />
            Télécharger
          </button>
          <button
            className="btn acc"
            disabled={busy || (mode === "photo" && !selPhoto) || (mode === "ai" && !aiUrl)}
            onClick={use}
          >
            {busy ? <span className="spin" /> : <Icon name="check" />}Utiliser comme visuel
          </button>
        </div>
      </div>
    </div>,
