...
export function LinkedInPostModal({ onClose }: { onClose: () => void }) {
  ...
  const [target, setTarget] = useState(''); // '' = profil personnel, sinon l'id de la page
  ...
  useEffect(() => {
    ...
    fetchLinkedInOrganizations(linkedinToken).then((d) => { if (d.available) setOrgs(d.organizations); });
  }, [linkedinToken]);
  ...
  const targetLabel = target ? orgs.find((o) => o.id === target)?.name || 'cette page' : (linkedinMe?.name ? `Profil : ${linkedinMe.name}` : 'Votre profil');
    ...
    <div className="kmodal" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="kmodal-card">
        <div className="kmodal-top">
          ...
          <div><h3>Publier sur LinkedIn</h3><div className="km-s">{targetLabel}</div></div>
          <button className="km-x" aria-label="Fermer" onClick={onClose}><Icon name="close" /></button>
        </div>
        <div className="kmodal-body">
          {orgs.length > 0 && (
            <div className="field">
              <label className="field-lbl">Publier en tant que</label>
              <select className="inp" value={target} onChange={(e) => setTarget(e.target.value)}>
                <option value="">{linkedinMe?.name || 'Mon profil personnel'}</option>
                {orgs.map((o) => <option key={o.id} value={o.id}>{o.name || `Page ${o.id}`}</option>)}
              </select>
            </div>
          )}
          <div className="field">
        ...
        <div className="kmodal-foot">
          <span className="grow" style={{ fontSize: 12.5, color: 'var(--tx-3)' }}>Publié en direct sur {target ? 'la page LinkedIn choisie' : 'votre profil LinkedIn'}.</span>
