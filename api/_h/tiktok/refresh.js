/* TikTok OAuth — refresh an expired access token using the refresh token.
   Accès protégé : session utilisateur (ou secret cron) obligatoire — sinon un
   refresh token volé suffirait à se faire frapper un access token frais par
   notre client_secret. POST uniquement : le refresh token voyage dans le
   corps, jamais en query string (logs d'accès). */
