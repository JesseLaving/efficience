/* POST /api/spaces/delete { spaceId } — permanently delete one of the
   signed-in user's own spaces. app_space_data cascades (ON DELETE CASCADE),
   so its stored snapshot (networks, contacts, calendar…) goes with it. */
...
export default async function handler(req, res) {
  ...
  try {
    const { rows } = await query(
      `DELETE FROM app_spaces WHERE id = $1 AND user_id = $2 RETURNING id`,
